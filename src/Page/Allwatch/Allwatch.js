...
import Allwatchswiper from "../Allwatch/Allwatchswiper"

import { useEffect } from "react"

import { useSelector, useDispatch } from "react-redux"
import {getAllwatch} from "../../store/Reducer/Allwatchreducer"
...
// console.log(products1);
    ...
    const {
        watchproducts,
        watchloading
    } = useSelector((state) => state.allwatch)
    const dispatch1 = useDispatch()


    useEffect(() => {
        dispatch1(getAllwatch())
    }, [])

    console.log(watchproducts);
    
    return (
        watchloading ? < div className = "loading-container-main" >
            <img className='loading' src={loadingimg} alt="" />

        </div> :
            <div className="container">
                <Link className='link' to="/">  Ana sehife  <img src={arrow} alt="" /> {watchproducts?.[0]?.categories?.[0]?.name}</Link>
                <div className='main-area'>
                    ...
                    <div className="part-1">
                        <div className="brend-container">
                            ...
                            <h5>{watchproducts.length} Məhsul tapıldı</h5>
