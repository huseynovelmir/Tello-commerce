...
import { getAllapplewatch } from "../../store/Reducer/applewatchreducer"
...
import Allapplewatchswiper from "./Applewatchswiper"
function Allapplewatch() {

    const {
        applewatchproducts,
        applewatchloading
    } = useSelector((state) => state.Allapplewatch)
    const dispatch3 = useDispatch()


    useEffect(() => {
        dispatch3(getAllapplewatch())
    }, [])

console.log(applewatchproducts);
...
console.log(click);
    return (
        applewatchloading ? < div className = "loading-container-main" >
            <img className='loading' src={loadingimg} alt="" />

        </div> :
            <div className="container">
                <Link className='link' to="/">  Ana sehife  <img src={arrow} alt="" /> {applewatchproducts?.[0]?.categories?.[2]?.name}</Link>
                <div className='main-area'>
                    ...
                    <div className="part-1">
                        <div className="brend-container">
                            ...
                            <h5>{applewatchproducts.length} Məhsul tapıldı</h5>
                    ...
                    < Allapplewatchswiper applewatchproducts = {
                    applewatchproducts
                    }
                    applewatchloading = {
                        applewatchloading
