...
import { getallqcy } from "../../store/Reducer/Allqcyreducer"
...
import Allqcyswiper from "./Allqcyswiper"
function Allqcy() {

    const {
        qcyproducts,
        qcyloading
    } = useSelector((state) => state.allqcy)
...
console.log(click);
    return (
        qcyloading ? < div className = "loading-container-main" >
            <img className='loading' src={loadingimg} alt="" />

        </div> :
            <div className="container">
                <Link className='link' to="/">  Ana sehife  <img src={arrow} alt="" /> {qcyproducts?.[0]?.categories?.[2]?.name}</Link>
                <div className='main-area'>
                    ...
                    <div className="part-1">
                        <div className="brend-container">
                            ...
                            <h5>{qcyproducts.length} Məhsul tapıldı</h5>
