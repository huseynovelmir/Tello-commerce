...
import Elementswiper from "../Allelement/Elementswiper"

import { useEffect } from "react"

import { useSelector, useDispatch } from "react-redux"
import { getElement } from "../../store/Reducer/notfoundreducer"
...
function Element() {
    ...
    const {
        elementproduct,
        elementloading
    } = useSelector((state) => state.allelement)
    const dispatch3 = useDispatch()


    useEffect(() => {
        dispatch3(getElement())
    }, [])
...
console.log(click);
    return (
        elementloading ? < div className = "loading-container-main" >
            <img className='loading' src={loadingimg} alt="" />

        </div> :
            <div className="container">
                <Link className='link' to="/">  Ana sehife  <img src={arrow} alt="" /> {elementproduct?.[0]?.categories?.[1]?.name}</Link>
                <div className='main-area'>
                    ...
                    <div className="part-1">
                        <div className="brend-container">
                            ...
                            <h5>{elementproduct.length} Məhsul tapıldı</h5>
